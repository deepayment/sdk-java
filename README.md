...
[`protocol/`](https://github.com/deepayment/sdk-java/tree/main/protocol/) is the source of truth, and the Go / JavaScript / PHP / Python / Java
...
```
implementation("com.deepayment:sdk:0.1.2")
```
...
import com.deepayment.sdk.DeepaymentException;
...
| --- | --- | --- |
| `DeepaymentException.Request` | Rejected **before it was sent** (local validation, a bad parameter, or a request the SDK could not encode or sign) | Safe to mark failed; fix the request and retry under the same `merchantOrderNo` |
| `DeepaymentException.Transport` | Handed to the transport, no usable response (connection failure, timeout, interrupted read) | Outcome unknown; **never mark a payout failed**. Query by `merchantOrderNo`, or resend the identical request under the same number |
| `DeepaymentException.Api` | The gateway returned a business error (`msg` / `apiMessage` / `traceId`) | Branch on `msg`. `IDEMPOTENCY_CONFLICT`: the number is taken but the platform could not return its order, query that number and keep querying rather than switching numbers. `CHANNEL_ERROR`: the order may already exist, query by `merchantOrderNo` first and reuse that number only once the query returns `ORDER_NOT_FOUND`. `CHANNEL_BUSY`: refused before the order was created, so resend the same number after a back-off; this is the only channel error that needs no query first |
| `DeepaymentException.Response` | The gateway or CDN returned something that is not an envelope (HTML 502, ...) | Outcome unknown; query before deciding |
| `DeepaymentException.ResponseTooLarge` | A response arrived but exceeded the size limit and was discarded | Outcome unknown; the order was most likely created, query before deciding |
...
[`protocol/merchant-api.md`](https://github.com/deepayment/sdk-java/blob/main/protocol/merchant-api.md#client-side-validation). Format checks
...
at <https://docs.deepayment.com>; its examples map one to one onto this SDK. The wire protocol is in [`protocol/webhook.md`](https://github.com/deepayment/sdk-java/blob/main/protocol/webhook.md) and
[`protocol/merchant-api.md`](https://github.com/deepayment/sdk-java/blob/main/protocol/merchant-api.md). Key points:
...
The tests assert directly against the vectors in [`protocol/testdata`](https://github.com/deepayment/sdk-java/tree/main/protocol/testdata/):
