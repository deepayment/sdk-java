...
- Documentation: `Client.Builder` javadoc explains each config field;
...
- Endpoints: `createPayment`, `createPayout`, `queryPaymentByOrderNo`, `queryPaymentByMerchantOrderNo`, `queryPayoutByOrderNo`, `queryPayoutByMerchantOrderNo`, `getBalance`, `getUsdRate`, `getPayoutReceipt`, `getPaymentCheckout`, `submitPaymentTradeNo`, `addPaymentExtraInfo`; every method returns `Map<String, Object>`.
- Local validation before signing: top-level required fields, decimal-string
  ...
  the gateway.
- Error taxonomy: `DeepaymentException.Request` (rejected before sending), `DeepaymentException.Transport` (sent or possibly sent, no response), `DeepaymentException.Api` (gateway business error), `DeepaymentException.Response` (non-envelope response).
