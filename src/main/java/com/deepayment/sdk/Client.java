...
public final class Client {
    ...
    /** Client configuration; a missing or invalid field makes build() throw DeepaymentException.Config. */
    public static final class Builder {
        ...
        public Client build() {
            ...
            if (baseUrl.isEmpty()) {
                throw new DeepaymentException.Config("sdk: baseUrl is required");
            }
            URI uri;
            try {
                uri = URI.create(baseUrl);
            } catch (IllegalArgumentException e) {
                throw new DeepaymentException.Config("sdk: baseUrl must be an absolute origin URL");
            }
            ...
            if (path != null && !path.isEmpty() && !path.equals("/")) {
                throw new DeepaymentException.Config("sdk: baseUrl must not contain a path");
            }
            ...
            if (accessKey.isEmpty()) {
                throw new DeepaymentException.Config("sdk: accessKey is required");
            }
            ...
            if (bodyKeyId.isEmpty()) {
                throw new DeepaymentException.Config("sdk: platformBodyKeyId is required");
            }
            ...
            if (rawWebhookKeys.isEmpty()) {
                throw new DeepaymentException.Config(
        ...
        private static byte[] decodeKey(String value, List<Integer> sizes, String what) {
            ...
            try {
                ...
                throw new DeepaymentException.Config("sdk: invalid " + what);
            }
    ...
    public Map<String, Object> submitPaymentTradeNo(String orderNo, String tradeNo) {
        ...
        if (no.isEmpty() || trade.isEmpty()) {
            throw new DeepaymentException.Request("sdk: invalid query parameter");
        }
    ...
    private static void validateAmount(String field, Object value) {
        if (value == null || "".equals(value)) {
            throw new DeepaymentException.Request("sdk: required field is empty: " + field);
        }
        if (!(value instanceof String text)) {
            throw new DeepaymentException.Request("sdk: " + field
                    + " must be a decimal string such as \"100.00\", got "
                    + value.getClass().getSimpleName());
        }
        if (!AMOUNT.matcher(text).matches()) {
            throw new DeepaymentException.Request(
                    "sdk: " + field + " must be a positive decimal string: '" + text + "'");
        }
        if (!NON_ZERO_DIGIT.matcher(text).find()) {
            throw new DeepaymentException.Request("sdk: " + field + " must be greater than 0");
        }
    }
    ...
    private static String validatePathOrderNo(String orderNo) {
        ...
        if (value.isEmpty()) {
            throw new DeepaymentException.Request("sdk: invalid path parameter");
        }
        return value;
    }
    ...
    private void validateMethod(String currency, Map<String, Object> method, Map<String, Rules.Rule> rules) {
        ...
        if (code.isEmpty()) {
            throw new DeepaymentException.Request("sdk: method code is required");
        }
        ...
        if (present.size() == 1 && want != null && !present.get(0).equals(want)) {
            throw new DeepaymentException.Request(String.format(
    ...
    public Map<String, Object> verifyWebhook(
        ...
        if (method == null || !method.toUpperCase(Locale.ROOT).equals("POST")) {
            throw new DeepaymentException.Webhook("sdk: webhook must be POST");
        }
        ...
        try {
            ...
            throw new DeepaymentException.Webhook("sdk: invalid webhook body");
        }
        if (!eventId.equals(payload.get("eventId"))) {
            throw new DeepaymentException.Webhook("sdk: webhook event id mismatch");
        }
        ...
        if (publicKey == null) {
            throw new DeepaymentException.Webhook("sdk: webhook platform key not found");
        }
    ...
    private Map<String, Object> write(String path, Map<String, Object> body, String idempotencyKey) {
        ...
        try {
            ...
            headers = built;
        } catch (DeepaymentException.Protocol e) {
            // Sealing and signing happen before the request is sent.
            throw new DeepaymentException.Request("sdk: build signed request: " + e.getMessage(), e);
        }
    ...
    private Map<String, Object> read(String path, Map<String, String> query) {
        ...
        query.forEach((key, value) -> {
            if (value == null || value.trim().isEmpty()) {
                throw new DeepaymentException.Request("sdk: invalid query parameter: " + key);
            }
            if (encoded.length() > 0) {
                encoded.append('&');
            }
    ...
    private Map<String, Object> decode(int httpStatus, byte[] raw) {
        if (raw.length > maxResponseBytes) {
            throw new DeepaymentException.ResponseTooLarge(
        ...
        try {
            ...
            throw new DeepaymentException.Response(httpStatus, raw);
        }
        ...
        if (httpStatus != 200 || codeValue != 200) {
            ...
            String message = data instanceof Map<?, ?> map && map.get("message") instanceof String s
                    ? s : "";
            throw new DeepaymentException.Api(
    ...
    private static final class HttpClientTransport implements Transport {
        ...
        public Map.Entry<Integer, byte[]> send(
            ...
            try {
                ...
                throw new DeepaymentException.Request("sdk: build http request: " + e.getMessage(), e);
            }
            try {
                ...
                throw new DeepaymentException.Transport("sdk: send request: " + e.getMessage(), e);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new DeepaymentException.Transport("sdk: send request interrupted", e);
            }
        }
    }
}
