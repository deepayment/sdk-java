...
public final class Protocol {
    ...
    private static String componentValue(
        ...
        if (name == null) {
            throw new DeepaymentException.InvalidHeader("unknown covered component: " + component);
        }
        ...
        if (!validHeaderValue(value)) {
            throw new DeepaymentException.InvalidHeader("missing or invalid header: " + name);
        }
        return value;
    }
    ...
    public static byte[] signatureBase(
        ...
        if (params.covered() == null || params.covered().isEmpty()) {
            throw new DeepaymentException.InvalidHeader("empty covered components");
        }
    ...
    public static void validateIdempotencyKey(String value) {
        if (!validUuidV4(value)) {
            throw new DeepaymentException.InvalidHeader(
    ...
    public static void validateWebhookEventId(String value) {
        if (value == null || !value.startsWith("evt_") || value.length() != 30) {
            throw new DeepaymentException.InvalidHeader("invalid Webhook-Event-Id");
        }
    ...
    public static void validateMerchantReadQuery(String rawQuery) {
        ...
        for (String pair : rawQuery.split("&")) {
            ...
            if (!ALLOWED_READ_QUERY_FIELDS.contains(key)
                    ...
                    || value.trim().isEmpty()) {
                throw new DeepaymentException.InvalidHeader("query field not allowed: " + key);
            }
            seen.put(key, true);
        }
    }
    ...
    public static void validateFreshness(SignatureParams params, long now) {
        if (params.created() <= 0 || params.expires() <= 0
            ...
            throw new DeepaymentException.ExpiredSignature("invalid signature lifetime");
        }
        if (now < params.created() - MAX_SIGNATURE_LIFETIME || now > params.expires()) {
            throw new DeepaymentException.ExpiredSignature("signature expired");
        }
    }
    ...
    public static void validateSignatureParams(SignatureParams params, long now) {
        if (params.label() == null || params.label().isEmpty()
            ...
            throw new DeepaymentException.InvalidHeader("invalid signature params");
        }
        if (SIGNATURE_LABEL_PLATFORM.equals(params.label())) {
            if (!validHeaderValue(params.keyId())) {
                throw new DeepaymentException.InvalidHeader("platform signature requires keyid");
            }
        } else if (!SIGNATURE_LABEL_MERCHANT.equals(params.label())) {
            throw new DeepaymentException.InvalidHeader("unknown signature label");
        }
        if (!coveredComponentsAllowed(params.label(), params.covered())) {
            throw new DeepaymentException.InvalidHeader("covered components not allowed");
        }
    ...
    public static SignatureParams parseSignatureInput(
        ...
        if (value == null || !value.startsWith(prefix)) {
            throw new DeepaymentException.InvalidHeader("bad Signature-Input label");
        }
        int closing = value.indexOf(')');
        if (closing < 0 || closing + 1 >= value.length() || value.charAt(closing + 1) != ';') {
            throw new DeepaymentException.InvalidHeader("bad Signature-Input components");
        }
        String[] rawComponents = value.substring(prefix.length(), closing).trim().split("\\s+");
        if (rawComponents.length != covered.size()) {
            throw new DeepaymentException.InvalidHeader("covered components mismatch");
        }
        ...
        if (parts.length != expected) {
            throw new DeepaymentException.InvalidHeader("bad Signature-Input params");
        }
        ...
        for (String key : got.keySet()) {
            if (!Set.of("created", "expires", "nonce", "keyid", "alg").contains(key)) {
                throw new DeepaymentException.InvalidHeader("unknown Signature-Input param: " + key);
            }
        }
    ...
    private static String unquote(String raw) {
        if (raw == null || raw.length() < 2 || raw.charAt(0) != '"'
            ...
            throw new DeepaymentException.InvalidHeader("bad quoted value: " + raw);
        }
    ...
    private static Ed25519PrivateKeyParameters privateKey(byte[] raw) {
        if (raw == null || (raw.length != 32 && raw.length != 64)) {
            throw new DeepaymentException.InvalidSignature(
    ...
    public static String signEd25519(byte[] privateKeyBytes, String label, byte[] base) {
        if (label == null || label.isEmpty() || base == null || base.length == 0) {
            throw new DeepaymentException.InvalidSignature("empty label or signature base");
        }
    ...
    public static void verifyEd25519(byte[] publicKey, byte[] base, byte[] signature) {
        if (publicKey == null || publicKey.length != ED25519_PUBLIC_KEY_SIZE
            ...
            throw new DeepaymentException.InvalidSignature("invalid ed25519 verify input");
        }
        ...
        try {
            ...
            throw new DeepaymentException.InvalidSignature("invalid ed25519 public key");
        }
        verifier.update(base, 0, base.length);
        if (!verifier.verifySignature(signature)) {
            throw new DeepaymentException.InvalidSignature("signature verification failed");
        }
    }
    ...
    public static String signatureHeader(String label, byte[] signature) {
        if (!validHeaderValue(label) || signature == null
            ...
            throw new DeepaymentException.InvalidSignature("invalid signature header input");
        }
    ...
    public static byte[] parseSignature(String value, String label) {
        if (!validHeaderValue(label)) {
            throw new DeepaymentException.InvalidSignature("invalid signature label");
        }
        String prefix = label + "=:";
        if (value == null || !value.startsWith(prefix) || !value.endsWith(":")) {
            throw new DeepaymentException.InvalidSignature("malformed Signature header");
        }
        byte[] signature;
        try {
    ...
    public static byte[] sealBodyEnvelope(byte[] plaintext, byte[] publicKey, String keyId) {
        if (plaintext == null || plaintext.length == 0 || plaintext.length > MAX_PLAIN_BODY_BYTES) {
            throw new DeepaymentException.InvalidEnvelope("invalid plaintext size");
        }
        if (publicKey == null || publicKey.length != X25519_PUBLIC_KEY_SIZE
                || !validHeaderValue(keyId)) {
            throw new DeepaymentException.InvalidEnvelope("invalid platform body key");
        }
    ...
    public static BodyEnvelope decodeBodyEnvelope(byte[] wireBody) {
        if (wireBody == null || wireBody.length == 0 || wireBody.length > MAX_WIRE_BODY_BYTES) {
            throw new DeepaymentException.InvalidEnvelope("invalid envelope size");
        }
        Map<String, Object> raw;
        try {
            raw = Json.readObject(wireBody);
        } catch (RuntimeException e) {
            throw new DeepaymentException.InvalidEnvelope("envelope is not valid json");
        }
        if (!raw.keySet().equals(Set.of("version", "alg", "keyId", "ciphertext"))) {
            throw new DeepaymentException.InvalidEnvelope("unexpected envelope fields");
        }
        ...
        if (!(version instanceof Number number) || number.intValue() != 1
            ...
            throw new DeepaymentException.InvalidEnvelope("invalid envelope");
        }
        try {
            Base64.getDecoder().decode(ciphertextValue);
        } catch (IllegalArgumentException e) {
            throw new DeepaymentException.InvalidEnvelope("invalid envelope ciphertext");
        }
    ...
    public static String peekBodyEnvelopeKeyId(byte[] wireBody) {
        ...
        } catch (DeepaymentException.InvalidEnvelope e) {
            return "";
        }
    }
    ...
    public static Map.Entry<byte[], String> openBodyEnvelope(
        ...
        if (publicKey == null || publicKey.length != X25519_PUBLIC_KEY_SIZE
            ...
            throw new DeepaymentException.InvalidEnvelope("invalid platform body key pair");
        }
        ...
        try {
            ...
            throw new DeepaymentException.InvalidEnvelope("envelope decrypt failed");
        }
